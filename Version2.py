    ...
    if query:
        try:
            ...
            if search_results and 'quotes' in search_results:
                matches = [(item['symbol'], item['shortname']) for item in search_results['quotes'] if 'symbol' in item and 'shortname' in item]
                if matches:
                    ...
                    for symbol, name in matches:
                        tk.Radiobutton(result_window, text=f"{symbol} - {name}", variable=result_var, value=symbol).pack(anchor='w')
...
start_date_entry = DateEntry(root, width=12, background='darkblue', foreground='white', borderwidth=2, year_bg='white', month_fg='black')
start_date_entry.grid(row=2, column=1, padx=10, pady=10)

tk.Label(root, text="End Date:").grid(row=3, column=0, padx=
10, pady=10)
end_date_entry = DateEntry(root, width=12, background='darkblue', foreground='white', borderwidth=2, year_bg='white', month_fg='black')
